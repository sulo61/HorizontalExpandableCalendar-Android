...
public class CalendarView extends RelativeLayout implements PageView.PageViewListener, Animations.AnimationsListener {
    ...
    private void setValuesFromAttr(AttributeSet attributeSet) {
        TypedArray typedArray = getContext().obtainStyledAttributes(attributeSet, R.styleable.CalendarView);
        if (typedArray != null) {
    ...
    private void setupBottomContainerFromAttr(@NotNull TypedArray typedArray) {
        if (typedArray.hasValue(R.styleable.CalendarView_bottom_container_height)) {
            ((LinearLayout.LayoutParams) findViewById(R.id.bottom_container).getLayoutParams()).height =
                    typedArray.getDimensionPixelSize(R.styleable.CalendarView_bottom_container_height,
                            LinearLayout.LayoutParams.WRAP_CONTENT);
        }
    }

    private void setupMiddleContainerFromAttr(@NotNull TypedArray typedArray) {
        if (typedArray.hasValue(R.styleable.CalendarView_center_container_expanded_height)) {
            Config.monthViewPagerHeight = typedArray.getDimensionPixelSize(
                    R.styleable.CalendarView_center_container_expanded_height, LinearLayout.LayoutParams.WRAP_CONTENT);
    ...
    private void setupTopContainerFromAttr(@NotNull TypedArray typedArray) {
        if (typedArray.hasValue(R.styleable.CalendarView_top_container_height)) {
            ((LinearLayout.LayoutParams) findViewById(R.id.top_container).getLayoutParams()).height =
                    typedArray.getDimensionPixelSize(R.styleable.CalendarView_top_container_height,
                            LinearLayout.LayoutParams.WRAP_CONTENT);
        }
    }

    private void setupExpandedFromAttr(@NotNull TypedArray typedArray) {
        if (typedArray.hasValue(R.styleable.CalendarView_expanded)) {
            Config.currentViewPager = typedArray.getBoolean(R.styleable.CalendarView_expanded, true) ?
